def getPost(id, cursor):
    selectQuery = ("SELECT id, company_id, source, content, likes_count, comments_count, retweets_count, created_time FROM posts WHERE id=%s")
    ...
    return {
            ...
            "retweets_count": post[6], 
            "created_time": post[7]
    }

def getPosts(cursor):
    selectQuery = ("SELECT id, company_id, content, source, likes_count, comments_count, retweets_count, created_time FROM posts")
